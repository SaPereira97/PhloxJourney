...
public class Pipe {
    ...
    public Pipe() {
        ...
        for (int i = 0; i < pipeDown.length; i++) {
            pipeDown[i] = new Picture(x[1], gap[i].getY() + 150, "resources/bamboo.png");
            pipeDown[i].draw();
            x[1] += 300;
        }
        for (int i = 0; i < pipeUp.length; i++) {
            pipeUp[i] = new Picture(x[2], gap[i].getY() - 732, "resources/bamboo2.png");
            pipeUp[i].draw();
            x[2] += 300;
        }
    }
    ...
    public void createPipe() {
        ...
        for (int i = 0; i < pipeDown.length; i++) {
            if (pipeDown[i].getX() == -300) {
                pipeDown[i].delete();
                pipeDown[i] = new Picture(900, gap[i].getY() + 150, "resources/bamboo.png");
                pipeDown[i].draw();
            }
        }
        for (int i = 0; i < pipeUp.length; i++) {
            if (pipeUp[i].getX() == -300) {
                pipeUp[i].delete();
                pipeUp[i] = new Picture(900, gap[i].getY() - 732, "resources/bamboo2.png");
                pipeUp[i].draw();
            }
        }
    }
